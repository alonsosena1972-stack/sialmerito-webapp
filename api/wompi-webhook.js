...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!secret || !event.event || !sig || !Array.isArray(sig.properties) || event.timestamp == null) {
    ...
    const source = values.join('') + String(event.timestamp) + secret;
    ...
    console.log(JSON.stringify({
      tag: 'WOMPI_PRODUCTION_WEBHOOK_VALID',
      event: event.event,
    ...
    return res.status(200).json({ received: true, environment: 'production' });
  } catch (_) {
